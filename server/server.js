...
// Route to fetch all news with a search query
...
// Route to fetch top headlines by country ISO code
...
//New route to fetch recent global news
app.get("/recent-news", async (req, res) => {
  const pageSize = parseInt(req.query.pageSize) || 80;
  const page = parseInt(req.query.page) || 1;
  const q = req.query.q || "news"; // Generic keyword to get global news

  const url = `https://newsapi.org/v2/everything?q=${encodeURIComponent(
    q
  )}&sortBy=publishedAt&page=${page}&pageSize=${pageSize}&apiKey=${
    process.env.API_KEY
  }`;
